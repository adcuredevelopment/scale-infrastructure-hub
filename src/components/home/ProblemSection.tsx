...
export const ProblemSection = () => {
  return (
    <section className="py-16 md:py-32 px-5 md:px-8 relative">
      <div className="absolute inset-0 bg-gradient-to-b from-background via-destructive/[0.02] to-background" />
      <div className="container mx-auto relative z-10">
        <ScrollReveal>
          <div className="text-center mb-10 md:mb-16">
            <span className="text-[11px] sm:text-xs font-medium text-destructive uppercase tracking-widest">The Problem</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          {problems.map((problem, i) => (
            <ScrollReveal key={problem.title} delay={i * 0.08}>
              <div className="group relative rounded-xl p-5 sm:p-6 md:p-8 hover-lift h-full bg-card/40 border border-destructive/10 hover:border-destructive/25 transition-all duration-500 overflow-hidden">
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-4 md:mb-6">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-destructive/10 flex items-center justify-center group-hover:bg-destructive/15 transition-colors duration-300">
                      <problem.icon className="w-5 h-5 md:w-6 md:h-6 text-destructive" />
                    </div>
                    <div className="text-right">
                      <div className="text-xl md:text-2xl font-display font-bold text-destructive/80">{problem.stat}</div>
                      <div className="text-[9px] md:text-[10px] text-muted-foreground/60">{problem.statLabel}</div>
                    </div>
                  </div>
                  <h3 className="font-display font-semibold text-base md:text-lg mb-2 md:mb-3">{problem.title}</h3>
