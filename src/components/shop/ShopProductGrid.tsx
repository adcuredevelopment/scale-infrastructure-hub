...
import { ShopCheckoutDialog, ShopCheckoutProduct } from "@/components/shop/ShopCheckoutDialog";
...
export const ShopProductGrid = ({ title, titleGradient, products, columns = 2 }: ShopProductGridProps) => {
  const [selected, setSelected] = useState<ShopCheckoutProduct | null>(null);
  ...
  return (
    <>
            ...
            {products.map((product, i) => (
              <ScrollReveal key={product.name} delay={i * 0.08}>
                <div className={`group relative rounded-xl p-5 sm:p-6 md:p-7 hover-lift h-full flex flex-col overflow-hidden transition-all duration-500 ${
                  product.popular
                    ? "border-2 border-primary/40 bg-card/80 backdrop-blur-xl mt-4 sm:mt-0"
                  ...
                  <div className="relative flex-1 flex flex-col">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-3 mb-2 md:mb-3">
                    ...
                    <Button
                      onClick={() => setSelected({ name: product.name, amount: product.amount })}
                      className="w-full min-h-[44px] group/btn"
      ...
      <ShopCheckoutDialog product={selected} onOpenChange={(open) => !open && setSelected(null)} />
    </>
  );
};
